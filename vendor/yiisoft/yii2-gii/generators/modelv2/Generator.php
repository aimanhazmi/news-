...
namespace yii\gii\generators\modelv2;
...
class Generator extends \yii\gii\Generator
{
    ...
    public $db                         = 'db';
    public $ns                         = 'app\mechanise\models';
    public $tableName;
    public $modelClass;
    ...
    private $serviceOutputPath     = "@app/modules/admin/service";
    private $controllerOutput      = "@app/modules/admin/controllers";
    private $modelOutPutPath       = "@app/modules/admin/models";
    private $managerViewOutPutPath = "@app/modules/admin/views";

    public $serviceSelfModel;
    public $serviceOriginalService;
    public $serviceName;

    public $parentModelClass;
    public $serviceClass;
    public $parentServiceClass;
    ...
    public function rules()
    {
        ...
        return array_merge(parent::rules(), [
            [['db', 'controllerClass', 'serviceClass', 'parentServiceClass', 'serviceName', 'parentModelClass', 'ns', 'tableName', 'modelClass', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass'], 'filter', 'filter' => 'trim'],
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            'parentModelClass'           => 'Parent Model',
            'controllerClass'            => 'Controller Name',
            'parentServiceClass'         => 'Parent Service',
            'ns'                         => 'Namespace',
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if (empty($this->modelClass)) {
                $this->modelClass = $this->convert2ClassName($tableName);
            } else {
                $this->modelClass = $this->convert2ClassName($this->modelClass);
            }

            if (!empty($this->parentServiceClass)) {
                $this->parentServiceClass = $this->convert2ClassName($this->parentServiceClass);
            }
            if (empty($this->serviceClass)) {
                $this->serviceClass = $this->modelClass;
            } else {
                $this->serviceClass = $this->convert2ClassName($this->serviceClass);
            }

            if (empty($this->parentModelClass)) {
                $this->parentModelClass = $this->modelClass;
            } else {
                $this->parentModelClass = $this->convert2ClassName($this->parentModelClass);
            }
            if (empty($this->controllerClass)) {
                $this->controllerClass = $this->modelClass;
            } else {
                $this->controllerClass = $this->convert2ClassName($this->controllerClass);
                $this->controllerClass = ucwords(strtolower($this->controllerClass));
            }

            $queryClassName = ($this->generateQuery) ? $this->generateQueryClassName($this->modelClass) : false;
            ...
            $files[]        = new CodeFile(
                Yii::getAlias($this->modelOutPutPath) . '/' . $this->modelClass . '.php',
                $this->render('model.php', $params)
            );

            $modulePath = Yii::getAlias($this->serviceOutputPath);
            $files[]    = new CodeFile(
                $modulePath . '/' . $this->serviceClass . 'Service.php',
                $this->render("service.php")
            );

            $controllerPath = Yii::getAlias($this->controllerOutput);
            $files[]        = new CodeFile(
                $controllerPath . '/' . ucwords(strtolower($this->controllerClass)) . 'Controller.php',
                $this->render("controller.php")
            );

            $files[] = new CodeFile(
                Yii::getAlias($this->managerViewOutPutPath) . '/' . strtolower($this->controllerClass) . '/manage.php',
                $this->render("manage.php")
            );

            $files[] = new CodeFile(
                Yii::getAlias($this->managerViewOutPutPath) . '/' . strtolower($this->controllerClass) . '/modify.php',
                $this->render("modify.php")
            );

//            $files[] = new CodeFile(
//                Yii::getAlias($this->managerViewOutPutPath) . '/' . strtolower($this->modelClass) . '/view.php',
...
//            );
            ...
            if ($queryClassName) {
                ...
                $params['modelClassName'] = $this->modelClass;
                $files[]                  = new CodeFile(
     ...
     * Date: 2018-02-17 11:41:05
     * Description: 把下划线转换为驼峰
     * @param      $str
     * @param bool $ucfirst
     * @return mixed|string
     */
    protected function convert2ClassName($str, $ucfirst = true)
    {
