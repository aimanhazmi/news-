$(function () {
    ...
    (function ($) {
        ...
        var items = [];
        // console.log(activeDom.length);
        if (activeDom.length) {
            var activeItems = $('#side-menu li.active');
            if (activeItems.length) {
                $.each(activeItems, function (i, n) {
                    var item = {};
                    item.title = $(n).find('.menu-title:eq(0)').html();
                    item.href = $(n).find('[href]:eq(0)').attr('href');
                    items.push(item);
                });
            }else{
        ...
        if (items.length) {
            ...
            var currentInfo = items.pop();
            console.log(currentInfo.title);
            $('[breadcrumbs-title]').html(currentInfo.title);
            var breadcrumbsPath = activeDom.attr('data-breadcrumbs-path');
            if (breadcrumbsPath != ''){
                $('[data-default=title]').html(breadcrumbsPath);
            }else{
                $('[data-default=title]').html(currentInfo.title);
            }

            if (items.length) {
                return;
                var html = [];
                var endHtml = '<li class="active">' + currentInfo.title + '</li>';
                items.forEach(function (value, index, array) {
                    html.push('<li><a href="' + value.href + '">' + value.title + '</a>&nbsp;&nbsp;<i class="fa fa-angle-right"></i>&nbsp;&nbsp;</li>');
                });
                html.push(endHtml);
                $('[breadcrumbs-posion]').after(html.join(''));
            }
        }
    })($);
    ...
    !(function () {
        ...
        function checking() {
            // console.log(window.location.host,window.location.protocol,window.location.port);
            $.ajax({
                url: '/admin/user/isoffline?iof=' + new Date().getTime(), dataType: 'json', success: function (data) {
                    if (data.code == 0) {
                        window.location.href = data.content.lockscreen_url;
                    }
                }
            });
        }
