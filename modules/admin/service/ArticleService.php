...
class ArticleService
{
     ...
     * @return mixed
     */
    public function search($params = [], $allowEmpty = false)
    {
        $model           = new SelfModel();
        ...
        $query->andOnCondition(['status' => 1]);

        if ($searchWord) {
            $query->andOnCondition(['like', 'art_title', $searchWord]);
          //文章搜索速度太慢，所以禁用  $query->orOnCondition(['like', 'art_content', $searchWord]);
        }
       ...
       // $orderBy[] = 'updated_at DESC';
        ...
        $itemsd = $query->select('id')->offset($paginolion->offset)->limit($paginolion->limit)->orderBy('updated_at DESC');     
        $items = SelfModel::find()->select('*')
        ->from(['t1' => $itemsd]) // 在这里使用了子查询 mender:coclu(bj_ljh)
        ->join('INNER JOIN',SelfModel::tableName(), 'article.id=t1.id')
        ->all();
        ...
        array_walk($items, function(&$item) use ($model) {
            ...
            $category              = CategoryService::getInstance()->findModel($item['category_id']);
            $item['category_name'] = $category->cate_name;
            
            $categorysallid         = CategoryService::getInstance()->search(['parent_id' => 1]);
            $categoryIdsallids       = ArrayToolkit::column($categorysallid['items'], 'id');

            $item['is_adv']      = in_array($category->parent_id, $categoryIdsallids );

            if (isset($item["art_content"])) {
                unset($item["art_content"]);
            }
    ...
    public function searchm($params = [], $allowEmpty = false)
    {
        $model           = new SelfModel();
        $attributeLabels = $model->attributeLabels();
        $query           = $model->getActiveQuery($params);
        ...
        if ($searchWord) {
            $query->orOnCondition(['like', 'art_title', $searchWord]);
          //文章搜索速度太慢，所以禁用  $query->orOnCondition(['like', 'art_content', $searchWord]);
        }
        $pageNo     = isset($params['page_no']) ? intval($params['page_no'] - 1) : 0;
        $pageSize   = isset($params['page_size']) ? intval($params['page_size']) : 10;
    ...
    public function getModelByIdf(int $id)
    {
        $model = $this->findModelf($id);
        return $model;
    }
    ...
    public function findModelf($id)
    {
        if (($model = SelfModel::find()->where(['id'=>$id,'status' => 1])->one()) !== null) {
            return $model;
        } else {
            throw new \Exception('该数据不存在.');
        }
