        ...
        <div class="panel">
            ...
            <div class="panel-body manage-container" id="manage-row">
                ...
                <div class="table-responsive-row">
                    <div class="table-responsive-left" v-show="!loading">
                        <table class="table table-hover table-striped table-advanced table-bordered table-border-radius">
                            ...
                            <tr v-cloak>
                                ...
                                <th v-for="(item, index) in tableThead" v-if="item.fixed == true">
                                    {{item.comment}}
                                    <i class="fa"
                            ...
                            <tbody>
                            <tr v-for="(item, index) in content.items" v-cloak>
                                ...
                                <template v-for="(theadItem,fixedField) in tableThead" v-if="theadItem.fixed == true">
                                    <td v-for="(val,field) in item" v-if="field == fixedField" v-html="val"></td>
                                </template>
                            </tbody>
                        </table>
                    </div>
                    <div class="table-responsive-right" v-show="!loading">
                        <table class="table table-hover table-striped table-advanced table-bordered table-border-radius">
                            <thead>
                            <tr v-cloak>
                                <th v-for="(item, index) in tableThead" v-if="item.fixed == 'right'">
                                    {{item.comment}}
                                    <i class="fa"
                                       v-if="item.option=='sort'"
                                       v-bind:class="[item.sortAsc ? 'fa-sort-desc' : 'fa-sort-asc']"
                            ...
                            <tbody>
                            <tr v-for="(item, index) in content.items" v-cloak>
                                <template v-for="(theadItem,fixedField) in tableThead"
                                          v-if="theadItem.fixed == 'right'">
                                    <td v-for="(val,field) in item" v-if="field == fixedField" v-html="val"></td>
                                </template>
                        ...
                        <table class="table table-hover table-striped table-advanced table-bordered table-border-radius">
                            ...
                            <tr v-for="(item, index) in content.items" v-show="!loading" v-cloak>
                                ...
                                <td v-if="theadSetting.btSelectAll" v-on:click="singleSelect(index)"
                                    style="text-align: center">
                                    <i class="fa"
                                       v-bind:class="[item.checked ? 'fa-check-circle-o' : 'fa-circle-o']"></i>
                                </td>


                                <td v-for="field in listBytableThead(item)" v-html="field"></td>

                                <td class="remove-inline-block-space" v-if="theadSetting.sort"
                                    style="text-align: center;">
...
<script type="text/javascript">
    ...
    var DataTable = {};
    DataTable.config = {
        baseUrl: "<?php echo Url::to('/admin/visitor') ?>",
        primaryKey: "<?php echo SelfModel::PRIMARY_KEY ?>",
        theadData: <?php echo json_encode($model->getVisitorManageTableHead()); ?>,
        ...
        btnSetting: {
            ...
            'dropdownBtn': {
                ...
                'menu': [
                    {'name': '设为黑名单', 'value': 'addBlacklist'},
                    {'name': '取消黑名单', 'value': 'cancelBlacklist'},
                    {'name': '禁言2分钟', 'value': 'levelA-Banned'},
                    {'name': '禁言5分钟', 'value': 'levelB-Banned'},
                    {'name': '禁言1天', 'value': 'levelC-Banned'},
                    {'name': '取消禁言', 'value': 'cancelBanned'},
                ]
            },
        },
