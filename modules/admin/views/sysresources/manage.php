        ...
        <div class="panel panel-white">
            ...
            <div class="panel-body manage-container" id="manage-row">
                    ...
                    <div class="col-md-4">
                        <div class="form-inline">
                            <div class="form-group">
                                ...
                                <a v-show="btnSetting.createBtn" v-on:click="createAction($event,'?t=1')" class="btn btn-green"
                                   style="display: none;"
                        ...
                        <form action="#" onsubmit="return false;" class="form-inline form-params" method="post">
                            <div class="form-group" v-if="btnSetting.searchBtn">
                                ...
                                <input id="relatedWord" name="searchWord" v-model="searchWord"
                                       v-on:keyup.enter="searchData" type="text" size="30"
                                       placeholder="请输入" class="ipt form-control">
                ...
                <table class="table table-hover table-striped table-advanced table-bordered table-border-radius">
                    ...
                    <tbody>
                    <tr v-for="(item, index) in content.items" v-show="!loading" style="display: none;">
                        ...
                        <td v-if="theadSetting.btSelectAll" v-on:click="singleSelect(index)" style="text-align: center">
                        ...
                        <td class="remove-inline-block-space" v-if="theadSetting.option" style="text-align: center;">
                            ...
                            <a v-on:click="callbackAction('changeShowStatus',item)"
                               class="btn btn-xs"
                               v-bind:class="[item._show != 1 ? 'btn-success' : ' btn-warning','btn btn-xs']"
                               href="javascript:void(0);">
                                <i class="fa"
                                   v-bind:class="[item._show != 1 ? 'fa-eye' : 'fa-eye-slash']">
                                {{item._show == 1?'隐藏':'展示'}}
                            </a>
                ...
                <div><p v-show="loading" id="loading" style="text-align: center">{{message}}</p></div>
...
<script type="text/javascript">
    ...
    function changeShowStatus(item) {
        console.table(item);
    }

    var DataTable = {};
    DataTable.config = {
        baseUrl: "<?php echo Url::to('/admin/sysresources') ?>",
        primaryKey: "<?php echo SelfModel::PRIMARY_KEY ?>",
        theadData: <?php echo json_encode($model->getManageTableHead()); ?>,
        ...
        callbacks: {
            changeShowStatus: function (app, item, callback) {
                var showType = item._show ? 0 : 1;
                $.ajax({
                    type: "PATCH",
                    url: DataTable.config.baseUrl + "/" + item.id,
                    data: {show: showType},
